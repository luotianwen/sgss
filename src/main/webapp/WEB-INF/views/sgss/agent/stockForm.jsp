...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/agent/stock/">库存列表</a></li>
		<li class="active"><a href="${ctx}/agent/stock/form?id=${stock.id}">库存<shiro:hasPermission name="agent:stock:edit">${not empty stock.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="agent:stock:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="stock" action="${ctx}/agent/stock/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="control-group">
			<label class="control-label">尺码2：</label>
			<div class="controls">
				<form:input path="uksize" htmlEscape="false" maxlength="255" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">尺码1：</label>
			<div class="controls">
				<form:input path="size" htmlEscape="false" maxlength="30" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
			<label class="control-label">库存数量：</label>
			<div class="controls">
				<form:input path="innernum" htmlEscape="false" maxlength="11" class="input-xlarge "/>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			<label class="control-label">货源名：</label>
			<div class="controls">
				<form:input path="warehousename" htmlEscape="false" maxlength="255" class="input-xlarge "/>
			</div>
		</div>
		<div class="form-actions">
			<shiro:hasPermission name="agent:stock:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
