...
public class FileUploadCommand extends Command implements IPostCommand {
    ...
    private boolean validateUploadItem(FileItem item, String path) {
        if (item.getName() != null && item.getName().length() > 0) {
            ...
            char[] arr$ = UNSAFE_FILE_NAME_CHARS;
            int len$ = arr$.length;
                    ...
                    try {
                        ...
                        if (FileUtils.checkFileSize((ResourceType)this.configuration.getTypes().get(this.type), item.getSize()) || this.configuration.checkSizeAfterScaling() && ImageUtils.isImage(file)) {
                            if (this.configuration.getSecureImageUploads() && ImageUtils.isImage(file) && !ImageUtils.checkImageFile(item)) {
                                this.errorCode = 204;
                                return false;
                            } else if (!FileUtils.checkIfFileIsHtmlFile(file.getName(), this.configuration) && FileUtils.detectHtml(item)) {
                                this.errorCode = 206;
                                return false;
                            } else {
                                return true;
    ...
    private String getFileItemName(FileItem item) {
       ...
       // return m.find() ? m.group(0) : "";
        ...
        return  String.valueOf(new Date().getTime()) + ( Math.random() + "" ).replace( ".", "" ).substring( 0, 3)+ "." + FileUtils.getFileExtension(origin);
    }
